...
stateDiagram-v2
    [*] --> understand
    [*] --> upload: local PDF
    upload --> parse: bounded copy, content identity
...
Uploads bypass understand/retrieve/select/fetch. Their stable identity is `upload:<SHA-256>`; filenames are display labels, never filesystem paths. Missing publication metadata stays empty. The upload and arXiv forms share parsing, retrieval, generation, validation and QA. Upload vector collections include the source identity so identical bytes imported through different entry points cannot reuse incompatible passage payloads.
...
- Encrypted PDFs, PDFs over 100 pages or with too little readable text fail explicitly. There is no silent partial-paper summary and no speculative OCR result.
